...
## The calibrated relaxation lookup

The attenuation of a voxel is delivered by relaxation parameters read from a calibrated lookup table. The table is a grid of the attenuation coefficient by the attenuation exponent, and this release calibrates two of them.

| table                                                      | mechanisms | cells | cells it marks invalid |
| ---------------------------------------------------------- | ---------- | ----- | ---------------------- |
| `relaxation_params_database_num_relax=4_20260828_1456.mat` | 4          | 1717  | 124                    |
| `relaxation_params_database_num_relax=3_20260828_1513.mat` | 3          | 1717  | 226                    |

The four mechanism table is the default. `fullwave/solver/shipped_database.py` is the one place the names and the counts are written, so a caller that wants the other one asks for it by count.

```python
from fullwave.solver.shipped_database import ShippedDatabase

ShippedDatabase.table               # the lookup the package defaults to
ShippedDatabase.mechanisms          # 4
ShippedDatabase.invalid_cells       # the record beside it
ShippedDatabase.stems               # every count this release ships
ShippedDatabase.table_of(3)         # the three mechanism lookup
ShippedDatabase.invalid_cells_of(3) # the record beside that one
```

A count the release does not calibrate raises rather than serving a table that was fitted for a different one.

The two mechanism table `relaxation_params_database_num_relax=2_20260113_0957.mat` is still in the package. It is kept so earlier work can be reproduced, and it is not part of this release. Name it by path to use it.

### Which cells are invalid, and why

The table does not serve every cell of its grid equally well. `ShippedDatabase.invalid_cells` is a JSON record naming every cell an evaluation found invalid, with the reason for each one. The lookup still serves an invalid cell and warns, so a run never stops on one.

| reason           | what failed                                                                  |
| ---------------- | ---------------------------------------------------------------------------- |
| `optimization`   | the calibration did not converge, so the table holds no fit for the cell     |
| `pml_reflection` | the absorbing layer returns more than the reflection gate                    |
| `attenuation`    | the delivered attenuation misses the analytical answer by more than the gate |
| `phase_velocity` | the delivered phase velocity misses by more than the gate                    |
| `diverged`       | the solver returned a field holding a value that is not a number             |

```python
from fullwave.utils.relaxation_parameters import RelaxationParametersGenerator

generator = RelaxationParametersGenerator()
generator.is_usable(alpha_coeff, alpha_power)   # False where a cell is invalid
generator.invalid_reasons()                     # every invalid cell, against its reasons
```

`is_calibrated` answers a narrower question, which is whether the fit converged. It reads the table alone and it needs no record. Give `path_invalid_cells` a record of your own when you bring your own table, because a record describes one grid.

### What the calibrated counts do not support

- **An air region needs two mechanisms.** A medium built with another count drops its air region and warns. Pass `n_relaxation_mechanisms=2` and the two mechanism table by path for that case.

One solver binary serves every mechanism count, because the kernel reads the count from `n_relax.dat` at run time rather than from its own name. The name carries the dimension and the CUDA version alone, as `fullwave2_2d_n_relax_multi_gpu_cuda124`.
