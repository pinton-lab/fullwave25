...
def main() -> None:
    ...
    sensor_output = load_data_with_time_step(
        file_path=sensor_output_path,
        n_sensors=sensor.ncoordsout,
        time_step=100,
    )
    # the retrieved sensor_output shape is [n_sensors, ]
    # in this case, we can reshape it back to the 2D grid shape for visualization
    ...
    # without loading the whole data into memory.
    sensor_output = load_data_with_sensor_index(
        file_path=sensor_output_path,
        n_sensors=sensor.ncoordsout,
        sensor_index=10,
    )
    # the retrieved sensor_output shape is [nt // sampling_modulus_time, ]
